...
class TagFilter:
    def __init__(self):
        ...
        QShortcut(QKeySequence(Qt.Key_Enter), self.filter_window) \
            .activated.connect(self.filter_addRow)
        ...
        QShortcut(QKeySequence(Qt.ControlModifier + Qt.Key_Enter), self.filter_window) \
            .activated.connect(self.filter_closeConfirm)
        ...
        QShortcut(QKeySequence(Qt.Key_Delete), self.filter_window) \
            .activated.connect(self.filter_deleteRow)
        # Esc  
        ...
        # 过滤器:标签过滤、图片过滤按钮对应两页
        self.filter_window.ui.pushbuttonGroup.buttonToggled.connect(self.filter_pageChanged)
        # 过滤器:黑白名单对应过滤模式
        self.filter_window.ui.radiobuttonGroup.buttonToggled.connect(self.filter_modeChanged)
        # 过滤器:添加
        self.filter_window.ui.filter_add.clicked.connect(self.filter_addRow)
        # 过滤器:删除
        self.filter_window.ui.filter_del.clicked.connect(self.filter_deleteRow)
        # 过滤器:确认
        self.filter_window.ui.filter_ok.clicked.connect(self.filter_closeConfirm)
        # 过滤器:取消
        self.filter_window.ui.filter_cancel.clicked.connect(self.filter_window.close)

        # fullInfo是一个嵌套字典，储存着不同工作路径下的全套信息
        self.filter_fullinfo = {}
        self.tagBlacklist = []
        self.tagWhitelist = []
        self.imageBlacklist = []
        self.imageWhitelist = []
    ...
    def filter_loadFile(self):
        try:
            # 载入4个字典
            with open('./cache/FilterInfo.txt', 'r') as f:
                self.filter_fullinfo = json.load(f)

            cur_fullInfo = self.filter_fullinfo[self.folder_path]
    ...
    def filter_saveFile(self):
        self.filter_fullinfo[self.folder_path] = {
        ...
        if not os.path.exists('./cache'):
            os.makedirs('./cache')
        with open('./cache/FilterInfo.txt', 'w') as f:
            json.dump(self.filter_fullinfo, f)

    def filter_loadTag(self):
        # 获取当前对应的过滤列表
    ...
    def filter_saveDict(self):
        if self.lastfullfilterState == '':
            return
        # 获取列表
        filter_list = self.filter_getTag()
        # 将内容保存到相应字典
    ...
    def filter_pageChanged(self):
        ...
        self.filter_modeChanged()

    def filter_modeChanged(self):
    ...
    def filter_apply(self, filterimage=False):
        # filterimage：是否应用图片筛选
        if filterimage == True:
            if self.curimagefilterMode == 'black':
                # 应用黑名单
                self.filter_imageBlacklist()
            elif self.curimagefilterMode == 'white':
                # 应用白名单
                self.filter_imageWhitelist()
        # 对标签的过滤始终执行
        if self.curtagfilterMode == 'black':
            self.filter_tagBlacklist()
        elif self.curtagfilterMode == 'white':
            self.filter_tagWhitelist()

    def filter_tagBlacklist(self):
    ...
    def filter_imageBlacklist(self):
        # 第一层：遍历图片列表
        for image_row in range(self.ui.imageTable.rowCount()):
            ...
            if item and item.text() != '':
                ...
                self.ui.imageTable.setRowHidden(image_row, row_hidden)

    def filter_imageWhitelist(self):
        for image_row in range(self.ui.imageTable.rowCount()):
            item = self.ui.imageTable.item(image_row, 1)
            if item and item.text() != '':
                image_filename = item.text()
